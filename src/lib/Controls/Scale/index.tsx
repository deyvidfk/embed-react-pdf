import { ElementType, FC, LabelHTMLAttributes, ReactNode, useCallback, useEffect, useMemo, useState } from "react";
...
const usePageFit = (documentInstance: false | pdfjs.PDFDocumentProxy | undefined) => {
    const windowWidth = useWindowWidth();
    const { scale } = useControls()

    const fitToWidth = useCallback((pageIndex:number=1) => {
        if (documentInstance) {
            documentInstance.getPage(pageIndex).then((page: any) => {
...
const ScaleControl: FC<TScaleControl> = ({ inputAs, onChange, label }) => {
    ...
    const labelRendered = useMemo(() => {
        ...
        if (typeof label == "string") {
            return <label className="mrc-embed-pdf__control-scale-label">{label}</label>
        }
        if (isLabelProps(label)) {
            return <label className="mrc-embed-pdf__control-scale-label" {...label?.props}>{label.value}</label>
        }
        return label
    }, [label])
