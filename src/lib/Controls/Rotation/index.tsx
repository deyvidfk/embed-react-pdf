import { FC, LabelHTMLAttributes, ReactNode, useEffect, useMemo, useState } from "react";
...
const RotateControl:FC<TRotateControl>=({ inputAs, onChange, label })=> {
    ...
    const labelRendered = useMemo(() => {
        ...
        if (typeof label == "string") {
            <label htmlFor="pdf-scale-control-zoom"> {label}: {rotate.value}</label>
        }
        if (isLabelProps(label)) {
            return <label className="mrc-embed-pdf__control-scale-label" {...label?.props}>{label.value}: {rotate.value}</label>
        }
        return label
    }, [label])
