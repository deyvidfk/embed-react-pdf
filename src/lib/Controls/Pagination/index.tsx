...
const PageInputControl: FC<TPageInputControl> = ({slots, onChange, inputAs }) => {

    const inputRef = useRef()

    const { pagination } = useControls()

    const { setCurrentPage, pagesCount, currentPage, } = usePagination({ pagesCount: pagination.total.value, page: pagination.page.value })
    ...
    const extraButtonProps = inputAs == "input" ?
        {
            type: "number", defaultValue: { currentPage }, ref: (inputRef as any),
            max: (pagesCount - 1), min: (0),
        }
        : {}

    const defaultButtonProps = {
        className: "mrc-embed-pdf__control-input",
        onKeyPress: (handleKeyPress), onBlur: (handleBlurPress),
        ...extraButtonProps
    }
