...
describe('Name of the group', () => {
    ...
    it("Deve renderizar uma pagina", async () => {
        ...
        const PageMocked = jest.fn().mockImplementation(() => {          
            return <>Pagina fake</>
        })
        ...
        const Wrapper: React.FC<PropsWithChildren<unknown>> = ({ children }) => {
            return <ControlsProvider pagination={{ total: 2, page: 1 }} rotate={90} scale={scaleValue}>{children}</ControlsProvider>
        }

        const { PageListItem } = await import("../PageListItem")

        render(<PageListItem id="page_list" index={indexValue} style={styleValue} />, { wrapper: Wrapper });

        const re = (document.getElementsByClassName("mrc-embed-pdf__page-list-item-wrap").item(0) as HTMLElement);
